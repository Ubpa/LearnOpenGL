...
Image::Pixel<uByte> Image::GetPixel_UB(size_t x, size_t y) {
	if (channel == 3)
		return { data[(y*width + x)*channel], data[(y*width + x)*channel + 1], data[(y*width + x)*channel + 2] };
	else if (channel == 4)
		return { data[(y*width + x)*channel], data[(y*width + x)*channel + 1], data[(y*width + x)*channel + 2],data[(y*width + x)*channel + 3] };
}
...
void Image::Free() {
	if (data != NULL) {
		switch (type)
		{
		case Ubpa::Image::ENUM_SRC_TYPE_NEW:
			delete[] data;
			break;
		case Ubpa::Image::ENUM_SRC_TYPE_STB:
			stbi_image_free(data);
			break;
		case Ubpa::Image::ENUM_SRC_TYPE_INVALID:
		default:
			break;
		}
	}
